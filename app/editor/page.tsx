  ...
  const saveCode = async () => {
    ...
    try {
      await addDoc(collection(db, `users/${user.uid}/snippets`), {
        ...
        name: `${language} - ${new Date().toLocaleTimeString()}`
      })
      await loadSnippets()
  ...
  return (
    <div className="p-4 bg-gray-950 min-h-screen text-white">
      ...
      <div className="grid grid-cols-2 gap-2 mt-2">
        <div>
          <div className="text-gray-400 text-sm mb-1">Input (stdin):</div>
          <textarea
            value={userInput}
            onChange={(e) => setUserInput(e.target.value)}
            placeholder="Input here..."
            className="w-full h-32 bg-gray-900 text-white p-2 font-mono rounded border border-gray-700 resize-none"
          />
        </div>
        <div>
          <div className="text-gray-400 text-sm mb-1">Output:</div>
          <div className="bg-black text-green-400 p-2 font-mono h-32 overflow-auto rounded border border-gray-700">
            <pre>{output}</pre>
          </div>
        </div>
      </div>
